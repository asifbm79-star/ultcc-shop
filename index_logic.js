document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCardData() {
        try {
            ...
            renderTable(allCards); // Initial render with all cards
        } catch (error) {
    ...
    function renderTable(cards) {
        ...
        cards.forEach(card => {
            const price = (Math.random() * (30 - 10) + 10).toFixed(2); // Random price between 10-30
            const row = `
                <tr>
                    <td>${card.bin}</td>
    ...
    // --- Dynamic Search/Filter Dropdowns (from previous step) ---
    ...
    function filterAndRender() {
        ...
        // NOTE: Bank and Zip filters are not connected to the live data yet. This is for UI demonstration.
        // A full implementation would require bank/zip data for each card in the database.
    ...
    // Add listeners to all filters to re-render the table on change
    [binInput, countrySelect, bankSelect, zipSelect].forEach(el => {
        el.addEventListener('input', filterAndRender); // 'input' for textbox
        el.addEventListener('change', filterAndRender); // 'change' for dropdowns
    });
